...
export const CategoryOnboardingModal = ({
  initialCategories = ["artificial-intelligence", "web-development", "cloud-computing"],
  onComplete,
  isDismissible = false,
  onClose,
}) => {
  ...
  const handleSelectAll = () => {
    setSelected(availableCategories.map((c) => c.id));
    setError(null);
  };
              ...
              >
                <div>
                  ...
                  <p className="text-[10px] text-[var(--ink-muted)] mt-1.5 leading-snug">
                    {cat.id === "artificial-intelligence" && "LLMs, foundational models, neural architectures & ML research."}
                    {cat.id === "web-development" && "Modern frontend frameworks, JavaScript, browsers & web engines."}
                    {cat.id === "cloud-computing" && "Distributed storage, consensus algorithms & cloud backends."}
                    {cat.id === "cyber-security" && "Vulnerability assessments, cryptography & zero trust protocols."}
                    {cat.id === "data-science" && "Data processing, analytics pipelines, statistical modeling & ML."}
